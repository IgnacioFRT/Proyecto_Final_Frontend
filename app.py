...
# --- VENTANA CALIDAD DE SERVICIO (QoS) ---
    ...
    try:
        with st.spinner('Evaluando disponibilidad y gaps de datos... ⏳'):
            ...
            # ==========================================
            # C. DETECCIÓN DE APAGONES (EL FILTRO PERFECTO)
            # ==========================================
            ...
            # 2. Creamos un DF limpio y medimos los saltos crudos
            ...
            for idx, row in cortes_graves.iterrows():
                ...
                # EL FILTRO DEFINITIVO QUE SOLUCIONA EL 03/03 vs 13/03
                # Si consumió casi nada (< 1.5 kWh) o el corte duró una barbaridad (> 4 horas)
                if energia_perdida < 1.5 or duracion > pd.Timedelta(hours=4):
                    ...
                    # B. Hundimos la gráfica a 0V obligatoriamente
                    mask = (df_grafico.index > inicio_corte) & (df_grafico.index < idx)
                    if mask.any():
                        df_grafico.loc[mask, ['UL1N', 'UL2N', 'UL3N']] = 0
                    else:
                        # Inyección forzada de 0V si InfluxDB mandó el salto totalmente vacío
                        df_grafico.loc[inicio_corte + pd.Timedelta(seconds=1), ['UL1N', 'UL2N', 'UL3N']] = 0
                        df_grafico.loc[idx - pd.Timedelta(seconds=1), ['UL1N', 'UL2N', 'UL3N']] = 0
                
            df_cortes = pd.DataFrame(lista_cortes)
            df_grafico = df_grafico.sort_index() # Ordenamos para que los puntos inyectados no rompan la línea
